...
if "current_user_id" not in st.session_state:
    ...
    with st.sidebar:
        ...
        <div style="text-align:center; padding: 12px 0 20px 0;">
            ...
            <div style="margin-top:8px; font-weight:700;">{esc(current_user['prenom'])} {esc(current_user['nom'])}</div>
            <div style="opacity:0.7; font-size:0.85rem; text-transform:uppercase;">{esc(current_user['role'])}</div>
        </div>
